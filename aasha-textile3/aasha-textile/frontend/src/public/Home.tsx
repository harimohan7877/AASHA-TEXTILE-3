...
import { ArrowRight, Truck, ShieldCheck, Award, MessageCircleHeart, Phone, MapPin, Play, BadgeCheck, Clock, Sparkles, Tag, ChevronRight, Package } from 'lucide-react';
import { useCategories, useProducts, useSettings, useVideos, useDrops, whatsappLink, slugify } from './usePublicData';
...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* VIDEO DROPS (5-Day Limited Release) */}
      {drops && drops.length > 0 && (
        <section className="pub-section pb-4">
          <div className="pub-container">
            <div className="bg-gradient-to-br from-stone-900 via-stone-900 to-stone-800 rounded-3xl p-6 sm:p-8 md:p-10 text-white shadow-xl border border-stone-800 relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-white/10 pb-6">
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-300 text-xs font-semibold uppercase tracking-wider mb-2">
                    <Sparkles size={13} className="text-amber-400 animate-pulse" />
                    Latest YouTube Video Drop
                  </div>
                  <h2 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-white">
                    {drops[0].title || 'Exclusive YouTube Drop'}
                  </h2>
                  <p className="text-stone-300 text-xs sm:text-sm mt-1 max-w-xl">
                    Order the exact fabric pieces shown in DevKrishna Sharma's latest video before the 5-day release expires.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <Link
                    to="/drops"
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold backdrop-blur border border-white/15 transition"
                  >
                    View All Drops ({drops.length}) <ArrowRight size={14} />
                  </Link>
                </div>
              </div>

              {/* Latest Drop Display: Video + Products Preview */}
              <div className="relative z-10 grid lg:grid-cols-12 gap-8 items-start">
                {/* Video Column */}
                <div className="lg:col-span-6 space-y-3">
                  <div className="relative aspect-video rounded-2xl overflow-hidden shadow-2xl bg-black border border-stone-800">
                    <iframe
                      src={`https://www.youtube-nocookie.com/embed/${drops[0].youtubeVideoId}?rel=0&modestbranding=1`}
                      title={drops[0].title}
                  ...
                  <div className="flex items-center justify-between text-xs text-stone-400 px-1">
                    <span className="flex items-center gap-1.5 text-amber-300 font-medium">
                      <Clock size={13} /> Active for 5 days only
                    </span>
                    <a
                      href={drops[0].youtubeUrl}
                      target="_blank"
                      rel="noreferrer"
                ...
                <div className="lg:col-span-6">
                  <div className="flex items-center justify-between mb-3 text-xs font-bold uppercase tracking-wider text-stone-400">
                    <span>Products in this Video ({drops[0].products?.length || 0})</span>
                    <Link to="/drops" className="text-amber-300 hover:underline">
                      See full drop →
                    </Link>
                  </div>

                  {!drops[0].products || drops[0].products.length === 0 ? (
                    <div className="p-8 rounded-2xl bg-white/5 border border-white/10 text-center space-y-2">
                      <Package size={28} className="mx-auto text-stone-400" />
                      <p className="text-xs text-stone-300">
                        Inquire directly on WhatsApp with screenshot from this video.
                      </p>
                      <a
                        href={whatsappLink(
                          settings?.whatsapp,
                          `Hi, I watched your video "${drops[0].title}" and want to inquire about wholesale price and availability.`
                        )}
                        target="_blank"
                        rel="noreferrer"
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {drops[0].products.slice(0, 3).map((prod) => (
                        <div
                          key={prod.id}
                          className="bg-white/10 rounded-2xl p-2.5 border border-white/10 flex flex-col justify-between hover:bg-white/15 transition group"
                        >
                          <div className="aspect-square rounded-xl overflow-hidden bg-black/40 mb-2">
                            {prod.imageUrl ? (
                              ...
                              <div className="w-full h-full grid place-items-center text-stone-500">
                                <Package size={24} />
                              </div>
                            )}
                          </div>
                          <div>
                            ...
                            <p className="text-xs font-semibold text-amber-300 mt-0.5">{prod.price}</p>
                            <a
                              href={whatsappLink(
                                settings?.whatsapp,
                                `Hi, I want to order "${prod.title}" (${prod.price}) shown in video "${drops[0].title}".`
                              )}
                              target="_blank"
                              rel="noreferrer"
                              className="mt-2 w-full py-1.5 px-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-semibold flex items-center justify-center gap-1 shadow-sm transition"
                            >
                              <WhatsAppIcon className="w-3 h-3" /> Order
                            </a>
                          </div>
                        </div>
                      ))}
